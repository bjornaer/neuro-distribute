        ...
        if (JSON.stringify(content_length_report) !== JSON.stringify({})) {
            ...
            for (let index = 0; index < content_length_report.stats.length; index++) {
                ...
                if (contentLengthCharts.hasOwnProperty(url_stats.safe_name)) {
                    contentLengthCharts[url_stats.safe_name].addValue([url_stats.peak_queue_size, url_stats.queue_size, url_stats.running_workers]);
                } else {
                    // If a chart doesn't already exist, create the chart first then add the value
                    contentLengthCharts[url_stats.safe_name] = new LocustLineChart($(".content-length-chart-container"), `Neuro API data for ${url_stats.safe_name}`, ["peak_queue_size", "queue_size", "running_workers"], "bytes");
